    ...
    try
      ...
      repeat
        ...
        begin
         ...
         if (pos(#13,packet)>0) or (Pos('<0>',packet)>0) then complete:=True;
          LineTokens.Clear;
      ...
      until (Pos(#13, packet) > 0) or (guard > 1000) or (Pos('<0>',packet)>0);
...
begin
if FileExists(GetUserDir + 'Desktop' + PathDelim + 'Chart.jpg') then
DeleteFile(GetUserDir + 'Desktop' + PathDelim + 'Chart.jpg'); //if exist delete it
...
Serials.LazSerial1.WriteData('BD0;TR1;FM1;TKM;DCH;'+sLineBreak);
...
begin
  ...
  timer1.Enabled:=False; //as saving the image deactivate timer no refresh need it
  try
  ...
  timer1.Enabled:=True; //activate again the timer
end;
