...
var
 ...
 row_idx:smallint;
 code:SmallInt;
 machine_Result,Actual_Center,Actual_span,Requested_Center,Requested_Span:Real;
 data:string;
begin //key preview enables by the form
//clear the collum 3,4
 ...
 { for i:=Low(show_parameters) to high(Show_parameters) do
  ...
  0..1: begin
   // for command_count:=1 to 2 do
    ...
    Actual_Center:=show_parameters[i].Value;
    Actual_Span:=show_parameters[i+1].Value;
    inc(i,2);
    inc(row_idx);
  end;
  2:begin
...
// write headers
 ...
 qtext.Caption:='Request Q =  '+FloatToStr(Filter_Q(Requested_Center,Requested_Span))+' | Actual  Q = '+FloatToStr(Filter_Q(Actual_Center,Actual_Span));
 end
 else
 qtext.Visible:=False;
...
begin
 ...
 if   Messagedlg('Enter Serial To Start','Enter Serial',mtInformation,[mbOK, mbCancel],0)=mrCancel then
 begin
   chkSave.Checked:=False;
   spSerial.Value:=0;
   spSerial.Visible:=False;
...
var
  ...
  show_Parameters:=DataModule1.read_show(trim(cboSelect.Items[cboSelect.ItemIndex]));
   ...
   inc(i,3);
    ...
    4: begin
    StringResult.cells[2,j]:='[ F0 + '+floatToStr(Show_parameters[i].Value)+Show_parameters[i].Aunit+' ] '+' '+floattoStr(Show_parameters[i+2].Value)+' '+Show_parameters[i+2].AUnit;;
         inc(j);
    StringResult.Rowcount:=j+1;
    StringResult.cells[1,j]:=Show_parameters[i].measurement_Type;
     StringResult.cells[2,j]:='[ F0 ' + floatToStr((-1)*Show_parameters[i+1].Value)+show_parameters[i+1].AUnit +' ] '+' '+floattoStr(Show_parameters[i+2].Value)+' '+Show_parameters[i+2].AUnit;
              inc(j);
    StringResult.Rowcount:=j+1;
   inc(i,3);
   end;
