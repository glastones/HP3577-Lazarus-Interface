...
uses
  ...
  DataBaseModule,LCLType, Buttons, Menus,hp3577,prologix,windows,misc_function,math;
...
var
 ...
 calibration_done:boolean;
  machine_Bandwith:Bandwidth_limits;
implementation
  uses Serial;
  procedure  sound_play;forward;
  procedure move_marker; forward;
{$R *.lfm}
...
//clear the collum 3,4 cells[0,0] headers
 machine_Result:=0;Actual_Center:=0;Actual_span:=0;code:=0;
 ...
 show_Parameters:=DataModule1.read_vna(trim(cboSelect.Items[cboSelect.ItemIndex]));
  ...
  hp3577.restore.span:=(hp3577.restore.span*2)+hp3577.restore.span;//restore span increased
  break;
 ...
 // begin
   prologix.connect;
     if not calibration_done then  begin
    for i:=Low(show_parameters) to high(Show_parameters) do
  begin
   ...
   ShowMessage('Calibration is over. Please put a Filter on the Jig and press ok.'+sLineBreak+' It will make auto scale finding');
   calibration_done:=True;
   frmTest.SetFocus;
   prologix.Write_Data('BD1;AB1;TKM;DMS;AB2');//check if filter is 50ohm or not
   repeat
         data:=serials.LazSerial1.ReadData;
         until data<>'';
         if data.Contains('OVER') then
         IS_50:=True
        else
        IS_50:=False;
     end;
      prologix.Write_Data('BD0;FM1;DCH');//initialize as fast as possible.
     if  IS_50 then //if 50 ohm
   hp3577.callibration.cal_scale:=0//no reference need it except if IL is too high
     else
     hp3577.callibration.cal_scale:=find_scale(Restore.center,Restore.span);//find the scale and make it reference.


  if not hp3577.calibration_proc.IL then //if not calibrate the start with
  ...
  0..1: begin
    ...
    stringResult.Cells[3,row_idx]:=FormatFloat('0.000',machine_result);
  ...
  8,19:begin
   machine_bandwith:=bandwidth_minus_plus(show_parameters[i].Value,show_parameters[i+1].Value,show_parameters[i+2].Value,show_parameters[i].measurement_code); //right = high freq
   stringResult.Cells[3,row_idx]:=FormatFloat('0.000',machine_bandwith.Right);
   code:=show_parameters[i].measurement_code;
   passFail(machine_bandwith.Right,code,row_idx);
   inc(row_idx);
   //machine_Bandwith:=bandwidth_minus_plus(show_parameters[i].Value,show_parameters[i+1].Value,show_parameters[i+2].Value);//left = Low freq
   stringResult.Cells[3,row_idx]:=FormatFloat('0.000',machine_bandwith.Left);
   code:=show_parameters[i].measurement_code;
   passFail(machine_bandwith.Left,code,row_idx);
   inc(i,3);
   inc(row_idx);
   StringResult.Invalidate;
  end;
   ...
   Application.ProcessMessages;
 if  chksave.Checked and not failed  then
 begin
 ...
 if (i=0) and chkSave.Checked then
 begin
// write headers
 ...
 if not failed and (info.filter_type=2) then
 begin
...
begin
  ...
  0..1,3:begin   //better than   {-3>-5 true}
  ...
  8,19: begin
     if machine>=(show_parameters[i].value/2)  then StringResult.Cells[4,row_idx]:='Pass'
      else
        begin
        failed:=True;
        StringResult.cells[4,row_idx]:='Failed';
        sound_play;TargetRow:=row_idx;
        StringResult.Invalidate; end;
     end;

//all gd
7,14,15,17,18: begin
    if machine>show_parameters[i].value  then begin failed:=True;StringResult.Cells[4,row_idx]:='Failed';sound_play;TargetRow:=row_idx; //StringResult.Invalidate;
    end
    else StringResult.cells[4,row_idx]:='Pass';
   end;
  ...
  begin
    if show_parameters[i].value<=machine  then begin failed:=True;StringResult.Cells[4,row_idx]:='Failed';sound_play;TargetRow:=row_idx; //StringResult.Invalidate;
    end
    else StringResult.cells[4,row_idx]:='Pass';
     end;
  13:begin
    ...
    begin
      ...
      else begin failed:=True;StringResult.Cells[4,row_idx]:='Failed';sound_play;TargetRow:=row_idx; //StringResult.Invalidate;
      end;
        end
  ...
  2:begin  // if FC =(fhihg+flow)/2
   ...
   begin failed:=True; StringResult.Cells[4,row_idx]:='Failed';sound_play;TargetRow:=row_idx;  //StringResult.Invalidate;
   end;
    end;
  9:begin
    //bw max                                                             bw min
    ...
    then begin failed:=True; StringResult.Cells[4,row_idx]:='Failed';sound_play;TargetRow:=row_idx; //StringResult.Invalidate;
    end
    else StringResult.Cells[4, row_idx] := 'Pass';
    end;
  end;
...
var
  ...
  show_Parameters:=DataModule1.read_show(trim(cboSelect.Items[cboSelect.ItemIndex]));
   ...
   8,19:
    begin
    StringResult.cells[2,j]:='[ F0 +'+floatToStr(ceil(Show_parameters[i].Value/2))+' '+Show_parameters[i].Aunit+' ] @ '+floatToStr(Show_parameters[i+1].Value)+' '+Show_parameters[i+1].Aunit;
        inc(j);
            StringResult.Rowcount:=j+1;
    StringResult.cells[2,j]:='[ F0 -'+floatToStr(ceil(Show_parameters[i].Value/2))+' '+Show_parameters[i].Aunit+' ] @ '+floatToStr(Show_parameters[i+1].Value)+' '+Show_parameters[i+1].Aunit;
    inc(j);
...
begin
 //collum autosize
  ...
  var
    ...
    command:='MKP '+IntToStr(i)+#10;
    serials.LazSerial1.WriteData(command);
    end;}
    R:=' ';i:=0; command:='';
    tim:=gettickcount64-tim;
    tim:=0;
   tim:=gettickcount64;
    while (R<>'') and (i<=400) do begin
    R:='';
    serials.LazSerial1.SynSer.Purge;
    command:='MKP '+IntToStr(i)+#10;
    serials.LazSerial1.WriteData(command);
    ...
    tim:=tim-gettickcount64;//ms
    showMessage('Stopped at '+IntToStr(i));
    end;
